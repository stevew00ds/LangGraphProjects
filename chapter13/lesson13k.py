...
# Supervisor prompt
system_prompt_fin = (
    "You are a Financial Supervisor managing the following agents: "
    "Portfolio Analysis, Market Research, and Risk Assessment."
...
def portfolio_analysis(query):
    ...
    try:
        ...
        response = (
            f"Portfolio Analysis for {stock_symbol}: "
            f"P/E Ratio: {metrics.get('peRatio')}, "
            f"Revenue Growth: {metrics.get('revenueGrowth')}, "
            f"52-Week High: {metrics.get('52WeekHigh')}, "
            f"52-Week Low: {metrics.get('52WeekLow')}"
        )
        return response
...
def market_research(query):
    ...
    response = "Latest Market News:\n" + "\n".join(
        f"{item['headline']} - {item['source']} ({item['url']})" 
...
def risk_assessment(query):
    """ Perform basic risk evaluation using volatility metrics """
    try:
        stock_symbol = query.split()[-1]
        quote = finnhub_client.quote(stock_symbol)
        price_change = quote.get("dp", 0)
        risk_level = "High Risk" if abs(price_change) > 5 else \
                    "Moderate Risk" if abs(price_change) > 2 else "Low Risk"
